...
func FileGetStringsWithPattern(path string, pattern string) (filelines []string, err error) {
	...
	for _, l := range lines {
		if r.MatchString(l) {
			...
			//nolint:gocritic // FindAllStringSubmatch says "A return value of nil indicates no match."
...
func FileWriteString(path, line string) (err error) {
	...
	err = os.Chmod(path, sixFourFour)
	if err != nil {
		return err
	}
	file, err = os.OpenFile(path, os.O_RDWR, sixFourFour)
	if err != nil {
		return err
	}
	defer file.Close()
...
func FileWriteStrings(path string, lines []string) (err error) {
	...
	for _, line := range lines {
		// trim if newline is already appended to string and always add newline
		_, err = w.WriteString(strings.Trim(line, "\n") + "\n")
		if err != nil {
			return err
		}
	}
...
func FileDelete(path string) error {
	...
	if !is {
		return errors.New("File doesn't exist " + path)
	}
